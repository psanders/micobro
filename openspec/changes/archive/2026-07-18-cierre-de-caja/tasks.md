...
- [x] 7.3 On-device walk: done 2026-07-18 against the real connected Datos sheet. Confirmed the system total (RD$3,300, all methods) on Cuadre; entered a mismatched verified total (RD$3,000) — "Cerrar caja" stayed disabled, "Diferencia: RD$300 de menos" shown; entered the matching total — button enabled; tapped it — total reset to RD$0, subtitle updated to "Desde el 18 jul · hace un momento", Desglose reset to 0. Push initially failed (`{"pushed":0,"failed":1}`) because this test lender's Sheet was already provisioned _before_ "Cierres" existed — `provisionSheet.ts` only creates tabs on first-ever connect, so already-connected lenders never get new entity tabs backfilled. This is a real, **pre-existing** gap (not introduced by this change — it already silently affected every past entity-tab addition, e.g. loan/payment/visit). Not fixed here: `addSheetTabs()` is destructive (deletes all existing tabs) when called on a populated sheet, so a proper fix needs a new additive-only "ensure tab exists" function — out of scope for this change. Manually added the "Cierres" tab (correct headers) to the test sheet via browser to unblock verification; filed issue #31 to track the underlying gap. After that, retried "Sincronizar ahora": `{"pushed":1,"failed":0}`, confirmed the exact row in the real sheet (id, amountCents=330000, periodStart empty, closedAt/createdAt correct)
