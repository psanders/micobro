...
The other half of the sync loop alongside push: pulls the lender's Google
Sheet back down into local SQLite, for `customers` and `loans` (the two
entities with an edit flow — `payments`/`visits` are append-only by product
design). The Sheet is a supported, first-class editing surface, not a
one-way backup: a lender who corrects a typo directly in the spreadsheet
expects that correction to reach the app. Conflict resolution is
remote-wins-with-guard — the Sheet wins on a conflicting field, except a
field with an unpushed local mutation, which stays local-authoritative until
it successfully pushes.
