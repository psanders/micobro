...
Local PIN protection for the app: a mandatory PIN created on first run,
required to unlock the app on every subsequent open. The PIN lives only on
the device (secure storage) — there is no username/password anywhere, and
Google Sign-In is a separate, optional backup concern (see `google-connect`).
