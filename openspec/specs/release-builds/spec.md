...
The EAS build configuration itself — which profiles exist, what each
produces per platform, where the project id lives, and the guarantee that a
clean checkout can run the `build:*` scripts in `package.json` without a
developer inventing missing config first.
