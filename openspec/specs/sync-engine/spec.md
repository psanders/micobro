...
Push-timing and retry policy for syncing local mutations to the lender's
Google Sheet: when a push runs (immediately after a mutation, on
reconnect, or manually), how concurrent triggers are serialized, and how
automatic vs. manual pushes differ in failure visibility. Builds on
`google-connect` (connect/disconnect and first-sync provisioning), which
this capability assumes is already established.
